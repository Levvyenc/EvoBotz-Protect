...
echo "🚀 Mengaktifkan Sistem Proteksi FileController (Cyber Defense Mode by Daffa)..."
...
class FileController extends ClientApiController
{
    ...
    private function checkServerAccess($request, Server $server)
    {
        ...
        if ($server->owner_id !== $user->id) {
            abort(403, '🚫 Access denied. This operation violates security policy enforced by Daffa.');
        }
    }
